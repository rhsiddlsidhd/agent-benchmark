...
```
src/context/
├── AdultContentContext.tsx   # Provider + 접근 훅 co-locate
└── ...                        # Context 1개당 파일 1개
```
...
- 값이 URL과 동기화되는 경우(예: 성인 콘텐츠 토글) 공유 컴포넌트에서 `useSearchParams`를 쓰지 않는다 — 앱 전체가 CSR bailout(Suspense 요구)돼 정적 페이지가 깨진다. `history.replaceState` 기반 얕은 갱신을 쓴다.
