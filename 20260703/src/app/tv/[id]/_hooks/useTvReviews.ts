...
/**
 * `useTvReviews` — TV 리뷰 온디맨드 조회 훅 (01_ARCHITECTURE §5.4 / FR-3).
 *
 * TanStack Query `useQuery` 로 `/api/tv/[id]/reviews` 를 TMDB 페이지(20개) 단위로
...
import type { ReviewsResponse } from "@/src/app/api/tv/[id]/reviews/_types";
...
export function useTvReviews(tvId: number, tmdbPage: number) {
  return useQuery({
    queryKey: ["tv-reviews", tvId, tmdbPage],
    queryFn: () => fetchTvReviews(tvId, tmdbPage),
    retry: 1,
  });
}
