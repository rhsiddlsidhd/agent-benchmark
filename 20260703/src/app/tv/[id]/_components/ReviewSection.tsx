...
/**
 * ReviewSection — TV 상세 리뷰 섹션(인라인, FR-3 확정 요구사항 / 01_ARCHITECTURE
 * §5.4, §4). movie/[id]/_components/ReviewSection.tsx 와 동일 패턴.
 *
 * TMDB 리뷰 API(`/tv/{id}/reviews`)는 페이지당 20개를 반환하지만, 요구사항은
...
export function ReviewSection({ tvId }: { tvId: number }) {
  ...
  const reviewsQuery = useTvReviews(tvId, tmdbPage);
