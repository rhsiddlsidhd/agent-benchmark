/**
 * `/api/tv/[id]/reviews` Route Handler (01_ARCHITECTURE §5.4, §4 / ADR-0003 / FR-3).
 *
 * TV 리뷰(TMDB `/tv/{id}/reviews`)를 프록시한다. 리뷰 페이지 이동은
 * 인터랙티브하므로 Client Component 가 이 핸들러를 경유해 온디맨드로 조회한다
 * (§4 — 인터랙티브 페치는 Route Handler 경유, `/api/movie/[id]/reviews` 와 동일
 ...
 * (정렬만 적용된 채로) 그대로 전달한다. 정렬/캐싱은 tmdb-client 의 `getTvReviews`
...
import { getTvReviews } from "@/src/lib/tmdb/client";
...
export async function GET(
  ...
  // TV id 는 1 이상 정수만 허용 — 아니면 TMDB 를 호출하지 않고 400 을 반환.
  if (!Number.isInteger(tvId) || tvId <= 0) {
    const body: ReviewsErrorResponse = { error: "잘못된 TV 식별자입니다." };
  ...
  try {
    const data: ReviewsResponse = await getTvReviews(tvId, page);
