 ...
 *
 * Client Component. T11 산출물(GenreFilter · useDiscoverInfinite · /api/discover)을
 * 하나의 화면으로 조립한다. 미디어 타입(영화/TV) 선택 + 장르 다중 선택에 따라
 ...
 * (T5/T6 검색 페이지의 sentinel 조립 패턴 재사용).
 *
...
import { GenreFilter } from "@/src/features/discover/genre-filter";
import { useDiscoverInfinite } from "@/src/features/discover/use-discover-infinite";
import { useIntersectionObserver } from "@/src/features/search/use-intersection-observer";
import type { Genre, MediaType, Movie, TVShow } from "@/src/lib/tmdb/types";

/** 포스터 그리드 반응형 열 수(03_DESIGN §4): 2→3→4→5→6열. spacing 은 card-gap 토큰만 사용. */
const DISCOVER_GRID =
 ...
 */
function toCardData(item: Movie | TVShow): CardData {
  if ("title" in item) {
    return {
      id: item.id,
      href: `/movie/${item.id}`,
 ...
 */
function buildQuery(type: MediaType, genreIds: number[]): string {
