...
- **인물 상세 필모그래피 타임라인 (예외)**: 위 "섹션당 1블록만 트리거, 아이템별 stagger 금지" 원칙의 이 기능 한정 예외. 진행선은 스크롤 진행률에 연동해 그려지는 드로잉 효과이며, 각 연도 노드는 스크롤 진입 시 개별적으로 활성화+페이드인한다. `prefers-reduced-motion`에서도 진행선 드로잉은 유지한다(스크롤 입력에 1:1로 매핑되는 값이라 자동재생 애니메이션이 아니므로) — 노드 쪽만 `fadeOnly`로 전환해 스프링·스케일 등 추가 트랜스폼을 제거한다(기존 `fadeOnly`/`fadeUp` 컨벤션과 동일 원칙).
