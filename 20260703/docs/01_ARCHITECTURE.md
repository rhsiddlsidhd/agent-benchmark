...
- DB 없음 (로그인/찜/리뷰 작성 등 개인화 기능이 Non-Goals이므로 영속 저장소 불필요 — TMDB 리뷰 읽기전용 열람(FR-8)은 매 요청 TMDB에서 페칭할 뿐 영속 저장은 하지 않는다)
...
- 인터페이스: `getPopularKrDramas()`, `getPopularKrVariety()`, `getPopularKrMovies()`, `getHeroCarouselItems()`, `getMovie(id)`, `getTvShow(id)`, `getPerson(id)`, `searchMulti(query)`, `getGenres(type)`, `getMovieReviews(id, page)`, `getTvReviews(id, page)`(FR-8) 등
- 의존성: TMDB REST API, `process.env.TMDB_API_KEY`
- 리뷰 조회 함수는 전역 `language` 고정값(ko-KR)을 요청 단위로 en-US로 오버라이드한다 — 근거·트레이드오프는 [ADR-0006](./02_ADR.md)
...
### 5.4 detail
- 책임: 영화/TV/인물 상세 + 연관 탐색 링크(출연진→인물, 필모그래피→작품) (FR-3, FR-4, FR-5), 영화/TV 상세 내 리뷰 섹션(FR-8)
- 인터페이스: `/movie/[id]`, `/tv/[id]`, `/person/[id]` 페이지. TV 상세 회차 선택은 `md` 뷰포트 기준으로 가로 필름스트립(모바일, `ScrollRail` 재사용)/세로 리스트(데스크톱, 백드롭과 2열)로 분기 렌더하며, 시즌탭·회차선택 전체에 roving tabindex(화살표 키 네비, 라우트 전용 `useRovingTabIndex` 훅) 공유 적용. 리뷰 섹션(`ReviewSection`, movie/tv 각 라우트 전용 Client Component)은 시즌선택과 동일하게 인터랙티브 온디맨드 조회 — 라우트별 `useMovieReviews`/`useTvReviews` 훅이 각자의 `/api/{movie|tv}/[id]/reviews` Route Handler를 경유하고, 렌더링 셸(`ReviewList`/`ReviewCard`)은 ui 모듈 공용
- 의존성: tmdb-client, ui

### 5.5 ui
- 책임: 공통 컴포넌트(카드, 이미지, 필터/토글, 애니메이션 래퍼, 드래그 스크롤 레일) — 디자인 토큰은 [03_DESIGN.md](./03_DESIGN.md) 참조
- 인터페이스: `<ContentCard>`(성인 콘텐츠 카드 단위 19+ 블러 게이트 담당, FR-7), `<PosterImage>`, `<GenreFilter>`, `<ScrollRail>`(내부적으로 `useDragScroll` 훅 사용, 홈 캐러셀 3종 + movie/tv 상세 레일 공용), `<ReviewCard>`/`<ReviewList>`(리뷰 렌더링 셸, FR-8 — 데이터 페칭 없이 props로만 상태를 받는 순수 렌더링 컴포넌트, movie/tv 양쪽 `ReviewSection`이 공유) 등
...
- **Movie/TVShow** ↔ **Review** (1:N, TMDB 리뷰 읽기전용 열람 — FR-8)
...
- **DB 없음**: 향후 찜/리뷰 작성 등 개인화 기능 확장 시 아키텍처 재설계 필요 — 현재 스코프에선 Non-Goal이라 수용(리뷰 읽기전용 열람은 FR-8로 이미 구현됨, DB 불필요)
