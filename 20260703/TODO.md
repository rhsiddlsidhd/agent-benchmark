...
## 필모그래피 타임라인 진행선이 스크롤 끝까지 안 채워짐

### 문제점

`/person/[id]` 필모그래피 타임라인(좌측 레일 진행선, `Filmography.tsx`)이 페이지 끝까지 스크롤해도 100% 안 채워지고 중간에 멈춤. 필모그래피가 짧은 인물(특히 "공개 예정" 구역이 없는 경우)일수록 훨씬 이른 지점에서 멈추는 게 체감됨.

### 원인

`useScroll({ target: timelineRef, offset: ["start end", "end start"] })`의 진행률 1 조건("end start")은 타임라인 **하단이 뷰포트 상단**까지 스크롤돼야 도달함. 그런데 타임라인 아래 남은 콘텐츠(예정 구역+푸터)가 뷰포트 높이보다 짧으면 브라우저가 스크롤 가능한 최대 위치가 그 지점에 못 미침 — 물리적으로 도달 불가능한 진행률 1을 목표로 계산하고 있었음. playwright로 person/500(Tom Cruise) 실측: 최대 스크롤(`scrollY=18332`)에서 `scaleY`가 `0.9868`에서 멈춤. 이 부족분(`뷰포트높이 − 타임라인 아래 남은 콘텐츠 높이`)은 필모그래피 길이와 무관한 고정값에 가까워서, 레일이 짧을수록 전체 대비 부족분 비중이 커짐.

### 해결

`src/app/person/[id]/_components/Filmography.tsx`의 `useScroll` offset을 `["start end", "end start"]` → `["start center", "end end"]`로 변경(진행률 1 조건을 타임라인 하단이 뷰포트 **하단**에 닿는 시점으로 바꿔, 아래 남은 콘텐츠 길이와 무관하게 항상 도달 가능하게 함).

**검증**: playwright로 최대 스크롤 위치에서 진행선 `computedStyle.transform`이 `none`(=identity, `scaleY(1)`)임을 확인. 중간 지점들(scrollY 0/1200/5000/10000/17800)에서도 값이 선형적으로 증가함을 실측.

## 필모그래피 타임라인 노드 등장이 진행선과 안 맞음

### 문제점

위 진행선 픽스 직후, 연도 노드(카드 묶음)의 페이드인이 `whileInView`(뷰포트 교차 30%) 기준이라 진행선이 실제로 그 노드를 지나가는 시점과 안 맞음 — 진행선은 이미 지나갔는데 카드가 아직 안 보이거나, 그 반대인 경우가 생김.

### 원인 1 — 임계값 계산이 항상 0으로 나옴

첫 시도로 각 노드(`FilmographyRevealNode`)가 자기 `useLayoutEffect`에서 `containerRef.current.offsetHeight`(부모 컨테이너 ref)를 읽어 `offsetTop/containerHeight` 임계값을 직접 계산하게 했더니, **모든 노드가 스크롤 0에서 즉시 노출**됐음. React가 레이아웃 이펙트를 자식→부모 순(post-order)으로 커밋하기 때문에, 자식의 레이아웃 이펙트가 실행되는 시점엔 부모 div의 `ref`(`timelineRef`)가 아직 안 붙어있어 `containerRef.current`가 항상 `null` → 임계값이 항상 0으로 폴백되고 있었음. playwright 콘솔 로그로 모든 노드에서 `hasContainer: false`임을 직접 확인해 특정.

### 원인 2 — React 재렌더 부하

원인 1을 고치려고 부모(`Filmography`)가 측정해서 `threshold` prop으로 내려주는 구조로 바꾸면서, 노드별 `useState`+`useMotionValueEvent`(scroll change마다 `setState`)로 반응하게 했더니 스크롤 프레임마다 노드 수(필모그래피 길수록 수십 개)만큼 React 재렌더가 발생 — 진행선(`style={{ scaleY: scrollYProgress }}`, React 재렌더 없이 motion value로 DOM을 직접 갱신)과 설계 원칙이 어긋남. 또한 `useState` 기반이라 "한 번 노출되면 유지"(비가역)만 가능해 위로 스크롤해도 카드가 다시 안 사라지는 문제도 있었음.

### 해결

`src/app/person/[id]/_components/FilmographyRevealNode.tsx`를 React state 없이 `useTransform`+`useSpring`으로 opacity/y를 `scrollYProgress`에서 직접 파생하도록 재작성(진행선과 동일하게 motion value 구독만으로 DOM 스타일 갱신 — 재렌더 없음, 위/아래 스크롤 양방향 자연 대응). 임계값 측정은 `src/app/person/[id]/_components/Filmography.tsx`의 부모 레이아웃 이펙트(`:scope > ol > li` 쿼리로 각 `li.offsetTop`을 컨테이너 높이로 나눔, 자식보다 나중에 커밋되므로 이 시점엔 컨테이너가 완성돼 있음)로 유지.

**검증**: playwright로 fresh reload마다 전 구간(scrollY 0/3000/6000/10000/14000/18332)에서 "진행선이 지나간 노드 = opacity>0.5인 노드" 일치(mismatch 0건) 확인. 아래로 스크롤 후 다시 위로 스크롤하면 노출됐던 카드 수가 줄어드는 것(양방향)도 확인.
