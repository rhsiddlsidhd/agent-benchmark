...
### 2.10 `ScrollRail` — 카드 레일 드래그 스크롤
- 홈 캐러셀 3종 + movie/tv 상세(출연진·추천작) 레일 공용 래퍼. 마우스/터치/펜 드래그(모멘텀), `←`/`→` 키보드, Tab 포커스 이동, 클릭/드래그 오인식 방지(5px 임계값)를 지원하는 `useDragScroll` 훅 기반.
- 카드 폭은 단일 반응형 스펙(`w-[42%] sm:w-[29%] md:w-[22%] lg:w-[15.5%] xl:w-[13.2%]`)만 지원 — 폭 override 없음. 이 스펙을 그대로 적용한 결과, 기존에 콘텐츠 카드보다 좁았던 출연진 `PersonAvatar` 레일(구 `33%→10%`)도 콘텐츠 카드와 동일 폭으로 통일됐다(의도된 디자인 통일).
...
- **히어로**: 트렌딩 기반 자동전환 캐러셀(3~5개, `HeroCarousel`) — `BackdropImage`(`16:9`, `min-h-420px`/`max-h-60vh`, 모바일은 `min-h-420px` 우선) + 하단 dot 인디케이터 + 하단 보호 그라데이션 위 오버라인·`display` 타이틀·개요 3줄 clamp·CTA. 자동전환 일시정지/재개 버튼 필수(WCAG 2.2.2, 6절 참고), `prefers-reduced-motion`이면 자동전환 타이머 자체를 실행하지 않는다.
- **캐러셀 섹션 반복**: "인기 드라마", "인기 예능", "인기 영화" 3종 — 섹션 헤더(`h2`) + `ScrollRail`(2.10) 기반 `ContentCard` 레일. 3섹션 간 세로 배치 순서는 미정([00_PRD.md §8](./00_PRD.md) 참고, 현재 문서 기재 순서로 잠정 배치).
...
- **필모그래피(타임라인)**: 출연·제작 참여를 하나의 연속 진행선으로 통합한다(출연작/제작참여 토글 없음). 좌측 고정 수직 레일 + 우측 연도별 카드 클러스터 레이아웃 — 모바일은 레일 폭만 축소하고 구조는 유지.
  - 정렬: 과거→현재 오름차순.
  - 연도 마커는 크레딧이 있는 연도만 표시한다(공백 연도는 건너뜀). 마커 간격은 균등이며 실제 시간축에 비례하지 않는다.
  - 개봉/방영일이 없는 작품(미공개·예정작)은 진행선 끝 "현재" 노드 다음의 별도 "예정" 구역에 모아 배치한다.
  - 각 카드(노드)는 역할 배지를 복수 표시한다(예: `[출연][감독]`). 캐릭터명·구체적인 job명은 상시 노출하지 않고 hover/tap 툴팁으로만 제공한다.
  - 한 연도의 카드 수가 가로 폭을 초과하면 `ScrollRail`(2.10)을 그대로 재사용한다(가로 드래그, 포스터 카드 폭 그대로 — 폭 override 없음).
  - 각 카드는 여전히 `/movie/[id]` · `/tv/[id]` 작품 상세로 링크한다(콘텐츠→인물→다른 작품 탐색 흐름 유지).
  - 필터/정렬 컨트롤은 두지 않는다 — 항상 통합 뷰 하나만 제공.
...
- **섹션/그리드 진입**: `staggerChildren: 0.04`로 카드 순차 등장(첫 뷰포트 1회, `viewport={{ once:true }}`). 섹션당 1블록(제목+콘텐츠 통째)이 트리거 단위이며, 내부 리스트/레일 아이템별 stagger는 하지 않는다.
- **인물 상세 필모그래피 타임라인 (예외)**: 위 "섹션당 1블록만 트리거, 아이템별 stagger 금지" 원칙의 이 기능 한정 예외. 진행선은 스크롤 진행률에 연동해 그려지는 드로잉 효과이며, 각 연도 노드는 진행선이 그 노드의 점 위치를 지나는 동안에만 활성화+페이드인한다 — **양방향**(스크롤을 올려 진행선이 다시 그 노드를 지나가면 카드도 다시 사라짐, `whileInView`의 1회성 등장과 다름)이며 React 재렌더 없이 motion value(스크롤 진행률)에서 직접 파생해(진행선과 동일 메커니즘) 노드가 아무리 많아도 스크롤 프레임마다 렌더 비용이 늘지 않는다. `prefers-reduced-motion`에서도 진행선 드로잉은 유지한다(스크롤 입력에 1:1로 매핑되는 값이라 자동재생 애니메이션이 아니므로) — 노드 쪽만 y 트랜스폼을 제거하고 opacity만 전환한다(기존 `fadeOnly`/`fadeUp` 컨벤션과 동일 원칙).
...
- **`ScrollRail`/히어로 드래그 스냅**: 드래그 종료 시 스프링(`{ type:'spring', stiffness:300, damping:30 }`)으로 가장 가까운 카드에 스냅. 빠른 드래그(속도 500px/s 초과)는 한 칸 더 이동(플릭). `prefers-reduced-motion`이면 스냅은 애니메이션 없이 즉시 이동, 히어로 자동전환은 타이머 자체를 실행하지 않는다(5절 원칙과 별개로 인터랙션 타이머 차원의 추가 조치).
...
- [ ] 논리적 탭 순서(헤더→필터→콘텐츠→푸터), 캐러셀(`ScrollRail`/히어로)은 `←`/`→` 방향키로 레일 이동 지원(필수, 2.10).
- [ ] 검색 제안·드롭다운은 Esc로 닫힘, 포커스 트랩 없는 자연스러운 이동.
- [ ] 히어로 자동전환 캐러셀은 일시정지/재개 버튼 필수(WCAG 2.2.2 Pause/Stop/Hide), `aria-pressed`로 상태 노출.
