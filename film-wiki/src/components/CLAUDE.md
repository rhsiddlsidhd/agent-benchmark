...
공용 UI(`components/`) — 특정 라우트에 종속되지 않는 재사용 프리미티브만 둔다. 도메인 로직·탐색(라우팅) 판단은 여기서 하지 않고 호출부 책임으로 남긴다.
...
- 파일명/export 식별자 케이스는 PascalCase다 — `ui/{컴포넌트명}.tsx` 형태로, 파일명은 export 컴포넌트명과 동일하게 쓴다(`ContentCard.tsx` → `export function ContentCard`). Global `~/.claude/docs/FRONTEND_FILE_CONVENTIONS.md`는 컴포넌트 조직 구조를 다루지 않으므로(프로젝트별 정의 영역) 이 규칙은 이 파일이 원본이다.
