...
- 파일명/export 식별자 케이스는 PascalCase다 — `ui/{컴포넌트명}.tsx` 형태로, 파일명은 export 컴포넌트명과 동일하게 쓴다(`ContentCard.tsx` → `export function ContentCard`). 컴포넌트 조직 구조는 프로젝트별 정의 영역이라 이 규칙은 이 파일이 원본이다.
