...
**주의(실측 확인됨): git 레포 루트 ≠ portfolio 프로젝트 루트.** 이 레포는 `portfolio`/`film-wiki` 등 여러 프로젝트를 함께 담은 모노레포라, 레포 루트는 portfolio 상위 디렉토리다. worktree는 항상 레포 전체를 체크아웃하므로, 생성된 worktree 경로 바로 밑에는 `frontend/`, `backend/`가 아니라 `portfolio/`(그리고 다른 프로젝트 폴더들)가 있다. 그래서 phase agent에게 전달하는 **"작업 디렉토리"는 `{worktree 경로}/portfolio`**여야 한다(worktree 경로 자체가 아님) — 이 안에서만 `path`(`frontend/api/` 등)가 그대로 풀린다. node_modules/`.venv` 심볼릭 링크도 이 하위(`{worktree 경로}/portfolio/frontend/node_modules` 등)에 건다. `git worktree remove`/`branch -d` 대상은 여전히 worktree 최상위 경로다(레포 단위 명령이라 프로젝트 서브경로와 무관).
