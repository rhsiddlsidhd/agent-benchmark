# CLAUDE.md — src/components/layouts/
...
- props 타입에 `src/types/*`의 도메인 타입을 쓰지 않는다 — `ui/`와 동일 이유(`src/components/ui/CLAUDE.md` 참고), 골격도 도메인에 종속되면 재사용 불가능해짐
...
- `use{Domain}` 훅이나 `services/*`, `api/*`를 직접 호출/import하지 않는다 — 이유: `src/components/ui/CLAUDE.md`와 동일, 골격에 fetch 로직이 섞이면 안 됨
